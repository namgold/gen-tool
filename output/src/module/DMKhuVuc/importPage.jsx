...
import { createMultiDMKhuVuc } from "./redux.js";
import EditModal from "./editModal.jsx";

class DMKhuVucImportPage extends React.Component {
    constructor(props) {
        super(props);
    ...
    save = (e) => {
        var doSave = (isOverride) => {
            ...
            this.props.createMultiDMKhuVuc(data, isOverride, (error, data) => {
                if (error) T.notify("Cập nhật dữ liệu bị lỗi!", "danger");
                else {
                    T.notify(`Cập nhật ${data && data.items ? data.items.length + ' ': ''}khu vực thành công!`, "success");
                    this.props.history.push("/user/danh-muc/khu-vuc");
                }
    ...
    changeActive = (item, key) => this.update({ma: item.ma}, {[key]: !item[key]}, () => T.notify("Cập nhật khu vực thành công!", "success"));
        ...
        return (
            <main className="app-content">
                <div className="app-title">
                    <div>
                        <h1><i className="fa fa-user" /> Tải lên file cập nhật khu vực</h1>
                        <p />
                    </div>
                    <ul className="app-breadcrumb breadcrumb">
                        ...
                        <li className="breadcrumb-item">Tải lên file cập nhật khu vực</li>
                    </ul>
                </div>
                <div className="row">
                    <div className="col-12 col-md-6 offset-md-3">
                        <div className="tile">
                            <div className="tile-body">
                                <FileBox ref={this.fileBox} postUrl="/user/upload" uploadType="DMKhuVucFile" userData="dmKhuVucImportData" style={{ width: "100%", backgroundColor: "#fdfdfd" }} success={this.onSuccess} error={this.onError} accept=".csv, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel" />
                                {this.state.message}
                            </div>
                            <div className="tile-footer text-right">
                                <a href="/download/SampleUploadDMKhuVuc.xlsx" className="btn btn-info">Tải xuống file mẫu</a>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <Link to="/user/danh-muc/khu-vuc" className="btn btn-secondary btn-circle" style={{ position: "fixed", bottom: "10px" }}>
...
const mapActionsToProps = { createMultiDMKhuVuc };
export default connect(mapStateToProps, mapActionsToProps)(DMKhuVucImportPage);
