    ...
    save = (e) => {
        ...
        if (this.state.isUpdate) {
            this.props.update({ ma: this.state.ma }, changes, (error, item) => {
                if (error) T.notify(error.message ? error.message : `Cập nhật giảm bảo hiểm xã hội ${changes.ma} bị lỗi!`, "danger");
                else {
                    T.notify(`Cập nhật giảm bảo hiểm xã hội ${changes.ma} thành công!`, "success");
            ...
            this.props.create(changes, (error, item) => {
                if (error) T.notify(error.message ? error.message : `Tạo mới giảm bảo hiểm xã hội ${changes.ma} bị lỗi!`, "danger");
                else {
                    T.notify(`Tạo mới giảm bảo hiểm xã hội ${changes.ma} thành công!`, "success");
                    $(this.modal.current).modal("hide");
                }
                this.setState({ saving: false });
            });
                ...
                <form className="modal-dialog modal-lg" role="document">
                    <div className="modal-content">
                        <div className="modal-header">
                            <h5 className="modal-title">{this.state.isUpdate ? "Cập nhật" : "Tạo mới"} giảm bảo hiểm xã hội</h5>
